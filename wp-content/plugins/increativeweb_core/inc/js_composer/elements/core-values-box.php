...
class WPBakeryShortCode_icw_core_values_box extends WPBakeryShortCode {
...
<div class="core-values-box <?php echo esc_attr( $wrapper_class ); ?>" <?php if( $animate_block == 'yes' && $animation_delay != '' ) { echo 'data-wow-delay="' . esc_attr( $animation_delay ) . '"'; } ?>>
  <figure> <img src="<?php echo esc_url( $icon_url ); ?>" alt="<?php echo esc_attr( get_the_title( $icon ) ); ?>"> </figure>
  <?php if( $title != '' ) { ?>
  <h6><?php echo esc_html( $title ); ?></h6>
  <?php } ?>
  <p><?php echo wp_kses_post( $description ); ?></p> </div>
<?php
...
vc_map( array(
  "base" => "icw_core_values_box",
  "name" => __( 'Core Values Box', 'ICWTHEME' ),
